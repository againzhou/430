...
{ 
 ...
 BCSCTL2 |= SELM1+SELM0;                // MCLK = LFXT1 (safe)SELM1表示XT2为时钟源
...
{
  TACTL = TASSEL0 + TACLR+ID0;              // ACLK,时钟源 clear清除TAR寄存器  n2 div二分频   
}

 void TIMEB_7(void)                              // timerb init (4 channel pwm)  
 {
  TBCTL = TBSSEL_1 + TBCLR+CNTL_3;      // ACLK, clear TBR  8bits 8位计数模式
 ...
 { 
    ...
    if (c1==0)
   	{ 
   		for (j1=0;j1<64;j1++)
   		{
     		CURRENTVALUEa1[j1]=CURRENT1A4[j1];             // 
...
{ 
	static u8 maf_f=0;
  	if(adcxa<4)                           // arrive position
    { 
    	if(maf_f<2) 
			maf_f+=1;
     	else
      	{
       		moveafinish=1;                      // set finish flag
      		h2=1;                                // little current  
      		maf_f=0;
      	}
  	}	
...
{  
  ...
  else if (mode==4)// D mode
  {
   ...
   if(pointer5==AN)
   {
       ...
       pointer5=800-AN;//pjz20070117 pointer5=(600-AN)+1;
     ...
     else if (mode==5) // huanse mode
  {
   ...
   else if (mode==9)                                 // huanse mode
  {
    if(zcj_huanse_select)
   {
   if(uartshu<11)
    {
     if(spi==0)
    ...
    { 
     if (jitou==1)                                         // zhen gao mode
     {
      if(jitoutest==0)
      {
       ...
       //P4OUT|=0x80;
       if(pointer3<37) pointer3+=1;                        // quxian
       if(((adcxa<11) &&(moveafinish==0)) || ((adcxb<11)&& (movebfinish==0)))
         { if(pointer3>2) pointer3-=2;
              }
         //pointer3=36;
   ...
   if(addreffect==1)                                   // 地址对 
     { //addreffect=0;
	   ...
	   if(crcshuju==1)
	    ...
	    { 
	      ...
	      */
	      ...
	      if((getchar1==0x73) && (jitou==1))	                                 //   
		  { 
			  goto end_rx_isr; 	 
           }      
	     
	    if((getchar1==0x74) && (jitou==1))// && a_lock1==0))
	       {if(moveafinish) putchar(adcvaluea>>2);                              // 发送 a 针高ad
		  
		    else putchar(0x62);
			goto end_rx_isr;
		   }
	     if((getchar1==0x75) && (jitou==1))// && b_lock1==0))                                   //发送 b 针高ad
	       {if(movebfinish) putchar(adcvalueb>>2);
		 
		    else putchar(0x62);
			goto end_rx_isr;
	       }
	    
	    if((getchar1==0x76) && (jitou==1))// && a_lock1==0))      //发送 a 针位置
	       {if(moveafinish) 
	       {
		  ...
		  if((getchar1==0x77) && (jitou==1))// && b_lock1==0))    //发送 b 针位置
		   ...
		   if((getchar1==0x7c) && (jitou==1))// && a_lock1==0))      //发送 a max
	       {if(moveafinish) 
	       {
		  ...
		  if((getchar1==0x7d) && (jitou==1))// && b_lock1==0))    //发送 a  min
	       {if(movebfinish)                                       
	       {
			...
			goto end_rx_isr;}
		   if((getchar1==0x7e) && (jitou==1))// && a_lock1==0))      //发送 b max
	       {
	        adcvaluea=ADC12MEM[4]>>2;
	        maxa=adcvaluea>>2;
	        mina=adcvaluea>>2;
	    ...
	    if((getchar1==0x7b) && (jitou==1))                                          //针高模式 清灯空走
	      {
	       gal();
		   gbl();
	   ...
	   //                           查询
	    ...
	    { jiandaoshuju1=1;
	     ...
	     goto end_rx_isr;} 
	      ...
	      if (((data1==0xb6) && (jianxian==1))||(data1==0xb4))                        // 松电数据        
	    { 
              if(zcj_huanse_select==1)
                banshuju=1;
              else
	  ...
	  if((jiandaoshuju==1) && (jianxian==1))                  // 收到剪刀数据指令
	   ...
	   if(zcj_huanse_select==0)
           {
	   if((SDshuju==1) && (jianxian==1))                      // 收到松电数据指令 08.12.02
	   {
	    ...
	    {crc1=getchar1;
         ...
         { jitoutest=0;
	      ...
	      if(((jitou==1) && (atzero==1)) && (startdo==0))
            ...
            if(buxiu_flage)
			  {
			   if(a_buxiu) posivaluea=posivalue[jiwei];
			   else posivaluea=posivalue[10];
			   if(b_buxiu) posivalueb=posivalue[jiwei];
			   else posivalueb=posivalue[10];
			   move1(0); 
...
{ 
 ...
 st:  headcode1=P5IN>>3;                // KNOW WHICH HEAD IT IS!
      headcode=headcode1|0xe0;
      if(headcode==0xe0)                                              // 07.2.8
      {headcode=0xde;}
      if(headcode==0xf0)
      {headcode=0xdf;} 
      
      zcj_huanse_select=P5IN>>1;
      zcj_huanse_select=zcj_huanse_select&0x1;//0--交流换色 1--H轴换色
     ...
     delay(1);
     tem=P5IN&0x03;  
     if(v_select!=tem) goto  st;
     else 
     {  for(i=0;i<38;i++)
           {timedata[i]=V_VALUE[0][i];}    
     } */
   ...
   // INTIALIZE VARIABLE
    ...
    adcvalueb=ADC12MEM[5]>>2; 
     ...
     //startdo=0;
     /*
  while(((headcode==0xde) || (headcode==0xdf))&&(( P5IN&0x02)==0x02))                                                //  老化测试用 板地址0x0f
     ...
     while(1)                                                           // start
      { //P4OUT^=0x40;
         ...
         //current_control(); 
          //putchar(0x10);
         //b_lock1=1;
       if(moveafinish & movebfinish)                                    // motor a,b stop
          {
          ...
          */
          ...
          if(startdo==0)
		  {
		  if((atzero==1)&&(tiaoshiflag==0)) test_switch();} 
